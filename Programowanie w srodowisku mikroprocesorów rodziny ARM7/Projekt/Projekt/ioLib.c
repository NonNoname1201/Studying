...
//@VD:
void LCDWriteBMP(unsigned char *bmp, unsigned char datasize, int x0, int y0, int x1, int y1)
{
  int x = (x0 < x1) ? x0 : x1;
  int y = (y0 < y1) ? y0 : y1;
  int dx = abs(x0 - x1);
  int dy = abs(y0 - y1);
  LCDPutStr(" ", x, y, SMALL, RED, WHITE, OPAQUE);
  LCDPutStr(" ", x + dx, y + dy, SMALL, GREEN, WHITE, OPAQUE);
  LCDClearScreen();
  ...
  //   if (sw == 0)
  //   {
  //     temp = bmp[j] * 0x100 + bmp[j + 1] & 0xf0;
  //     sw = 1;
  //   }
  //   else
  //   {
  //     temp = (bmp[j] & 0x0f) * 0x100 + bmp[j + 1];
  //     sw = 0;
  //   }
  //   LCDSetPixel(j % dx + x, j / dy + y, temp);
  // }
