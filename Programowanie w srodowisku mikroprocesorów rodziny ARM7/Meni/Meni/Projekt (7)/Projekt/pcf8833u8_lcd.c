...
void WriteSpiData(unsigned int data)
{
  ...
  data = (data | 0x0100); // 0b1_0000_0000
...
//
//@VD:
void LCDPutCharExtended(char c, int x, int y, int size, int fColor, int bColor, int transparentMode)
{
  ...
  for (i = nRows - 1; i >= 0; i--)
  {
    ...
    for (j = 0; j < nCols; j++)
    {
      ...
      // now get the pixel color for the pixel
      ...
      case OPAQUE:
        ...
        WriteSpiData((Word0 & 0xF) << 4);
        break;
      case TRANSPARENTTEXT:
        if (Word0 == bColor) // write only BG
        {
          WriteSpiData((Word0 >> 4) & 0xFF);
          WriteSpiData((Word0 & 0xF) << 4);
        }
        break;
      case TRANSPARENTBG:
        if (Word0 != bColor) // write only text
        {
          WriteSpiData((Word0 >> 4) & 0xFF);
          WriteSpiData((Word0 & 0xF) << 4);
        }
...
//@VD:
void LCDPutCharExtendedCustom(int addressOfChar, int x, int y, unsigned char *font, int fColor, int bColor, int transparentMode)
{
  unsigned int nColumns = *font;  // Width of the font in pixels
  unsigned int nRows = *font + 1; // Height of the font in pixels
  unsigned int nBytes = font + 2; // Number of bytes in the font array

  unsigned char dataRow; // Holds the ROW data that will be written to the LCD

  unsigned char mask; // Holds the bit mask for the current pixel in the current row

  unsigned int mainColor; // Holds the color of the current pixel in the current row
  ...
  {
    dataRow = font + addressOfChar * nColumns + i; // Get the current row of data from the font array

    mask = 1 << (nColumns - 1);

    // Set the mask to the MSB of the current row of data
    ...
    for (int j = 0; j < nColumns; j++)
    {
      ...
      mask = mask >> 1; // Shift the mask to the next pixel

      switch (transparentMode)
      {
      case OPAQUE:
        WriteSpiData((mainColor >> 4) & 0xFF);
        WriteSpiData((mainColor & 0xF) << 4);
        break;
      case TRANSPARENTTEXT:
        if (mainColor == bColor) // write only BG
        {
          WriteSpiData((mainColor >> 4) & 0xFF);
          WriteSpiData((mainColor & 0xF) << 4);
        }
        break;
      case TRANSPARENTBG:
        if (mainColor != bColor) // write only text
        {
          WriteSpiData((mainColor >> 4) & 0xFF);
          WriteSpiData((mainColor & 0xF) << 4);
        }
...
void LCDPutStr(char *pString, int x, int y, int Size, int fColor, int bColor, int transparentMode)
{
  ...
  while (*pString != 0x00)
  {
    ...
    // draw the character
    LCDPutCharExtended(*pString++, x, y, Size, fColor, bColor, transparentMode);
