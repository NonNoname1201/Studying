...
void LCDPutChar(int addressOfChar, int x, int y, int fontType, int fColor, int bColor, int transparentMode)
{
  unsigned char *FontTable[] = {(unsigned char *)FONT6x8, (unsigned char *)FONT8x8,
                                (unsigned char *)FONT8x16, (unsigned char *)FontCustom)};
  unsigned char *font = FontTable[fontType]; // Pointer to the desired font table
  addressOfChar += (fontType == FontCustom) ? 1 : -31;
  ...
  unsigned int secondaryColor; // Holds the color of the current pixel in the current row

  for (int i = 0; i < nRows; i++)
  {
    dataRow = font + addressOfChar * nColumns + i; // Get the current row of data from the font array

    mask = 1 << (nColumns - 1); // Set the mask to the MSB of the current row of data
    ...
    else
    {
      ...
      if (transparentMode == TRANSPARENTTEXT)
      {
        if (mainColor == bColor) // write only BG
        {
          LCDSetPixel(x + j, y + i, mainColor);
        }
      }
